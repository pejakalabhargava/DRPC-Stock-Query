...
public class DRPCStateQueryTopology {
	...
	public static void main(String[] args) throws Exception {
		...
		// The 1st arg in drpc.execute(): function name == first arg in
		// newDRPCStrem()
		// The 2nd arg: field tagged as "args" that needs to be parsed
		for (int i = 0; i < 10; i++) {
			System.out
					.println("DRPC RESULT: "
							+ drpc.execute("count_per_actors_event",
									"dave dave nathan"));
			Thread.sleep(1000);
		}
		...
		// System.out.println(client.execute("count_per_actor", "dave nathan"));
	}

	private static StormTopology buildTopology(LocalDRPC drpc)
			throws IOException {
		TridentTopology topology = new TridentTopology();
		FakeTweetsBatchSpout spoutFakeTweets = new FakeTweetsBatchSpout(
				DATA_PATH);
		// Build a (key,value) persistent in-memory hash map: countState
		// from the fake tweets spout
		TridentState countStateDBMS = topology
		...
		topology.newDRPCStream("count_per_actors_event", drpc)
				...
				.aggregate(new Fields("per_actor_count"), new Sum(),
						new Fields("sum"));
